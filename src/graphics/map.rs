...
fn shader() -> Shader {
    ...
    const FRAG_SHADER: &str = r#"
        ...
        void main() {
            vec4 v = texture(sampler, midtex * textureScale);
            float trans = texture(samplerMap, midtex).r;
            final_color = color * vec4(v.x, v.y, v.z, 1) * v.w * trans;
        }
    "#;
    ...
    Shader {
        ...
        u_sampler_map: get_uniform(shader_program, "samplerMap"),
        u_texture_scale: get_uniform(shader_program, "textureScale"),
    }
}
...
impl Renderer {
    ...
    pub fn render(&self, matrix: &Matrix4<f32>, color: &Vector4<f32>, texture: &Texture, texture_map: &Texture, texture_scale: f32, range: VertexRange) {
        unsafe {
            ...
            glUniform1i(self.shader.u_sampler_map, 1);
            glUniform1f(self.shader.u_texture_scale, texture_scale);
        }
        texture.bind_to(0);
        texture_map.bind_to(1);
